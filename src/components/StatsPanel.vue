...
const chartOptions = ref({
  chart: {
    ...
    foreColor: '#8a99af', // Color de los textos de los ejes (gris claro) — token: --bello-text-muted
  },
  theme: {
  ...
  // El color base (púrpura — token: --bello-purple)
  colors: ['#9b7dff'],
  ...
  fill: {
    ...
    gradient: {
      ...
      type: 'horizontal',
      shadeIntensity: 0.5,
      gradientToColors: ['#fff2c7'], // Termina en crema — token: --bello-cream
      inverseColors: true,
      opacityFrom: 1,
      opacityTo: 0.9,
      stops: [0, 100],
  ...
  dropShadow: {
    ...
    left: 0,
    blur: 5,
    color: '#9b7dff', // El brillo del mismo color que la barra
    opacity: 0.5,
  },
...
.sliding-panel {
  ...
  background: var(--bello-surface-2);
...
.giant-number {
  ...
  text-shadow: 0 0 10px rgba(155, 125, 255, 0.4);
  margin: 0;
}
</style>
